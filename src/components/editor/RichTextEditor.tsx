  ...
  return (
    <button
      ...
      className={`rounded-lg p-1.5 transition-colors ${
        active ? "bg-slu-blue text-white" : "text-slu-gray-500 hover:bg-slu-gray-100 hover:text-slu-black"
      }`}
    >
      {children}
    </button>
...
export function RichTextEditor({ value, onChange, placeholder, accept = "image/*" }: RichTextEditorProps) {
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-xl border border-slu-gray-200 bg-white" style={{ height: "500px" }}>
      ...
      >
        <ToolbarButton onClick={() => editor.chain().focus().undo().run()} title="Undo"><UndoIcon /></ToolbarButton>
        <ToolbarButton onClick={() => editor.chain().focus().redo().run()} title="Redo"><RedoIcon /></ToolbarButton>
        <ToolbarDivider />

        <ToolbarButton onClick={() => editor.chain().focus().toggleBold().run()} active={editor.isActive("bold")} title="Bold"><BoldIcon /></ToolbarButton>
        <ToolbarButton onClick={() => editor.chain().focus().toggleItalic().run()} active={editor.isActive("italic")} title="Italic"><ItalicIcon /></ToolbarButton>
        <ToolbarButton onClick={() => editor.chain().focus().toggleUnderline().run()} active={editor.isActive("underline")} title="Underline"><UnderlineIcon /></ToolbarButton>
        <ToolbarButton onClick={() => editor.chain().focus().toggleStrike().run()} active={editor.isActive("strike")} title="Strikethrough"><StrikethroughIcon /></ToolbarButton>
        ...
        <ToolbarButton onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} active={editor.isActive("heading", { level: 1 })} title="Heading 1"><H1Icon /></ToolbarButton>
        <ToolbarButton onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} active={editor.isActive("heading", { level: 2 })} title="Heading 2"><H2Icon /></ToolbarButton>
        <ToolbarButton onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()} active={editor.isActive("heading", { level: 3 })} title="Heading 3"><H3Icon /></ToolbarButton>
        <ToolbarDivider />

        <ToolbarButton onClick={() => editor.chain().focus().toggleBulletList().run()} active={editor.isActive("bulletList")} title="Bullet list"><ListUnorderedIcon /></ToolbarButton>
        <ToolbarButton onClick={() => editor.chain().focus().toggleOrderedList().run()} active={editor.isActive("orderedList")} title="Numbered list"><ListOrderedIcon /></ToolbarButton>
        ...
        <ToolbarButton onClick={() => editor.chain().focus().toggleCodeBlock().run()} active={editor.isActive("codeBlock")} title="Code block"><CodeBlockIcon /></ToolbarButton>
        <ToolbarDivider />
        ...
        <ToolbarButton onClick={() => editor.chain().focus().setHorizontalRule().run()} title="Horizontal rule"><HorizontalRuleIcon /></ToolbarButton>
        <ToolbarButton onClick={handleImageUpload} title="Insert image"><ImageIcon /></ToolbarButton>
        <ToolbarButton onClick={() => editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()} title="Insert table"><TableIcon /></ToolbarButton>
      </div>
      ...
      {editor.isActive("table") && (
        <div className="flex-none flex items-center gap-1 border-b border-slu-gray-200 bg-slu-gray-50 px-3 py-1.5 text-xs text-slu-gray-500">
          ...
          {[
            ...
            <button key={item.label} type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => item.fn()} className="rounded px-2 py-1 hover:bg-slu-gray-100">{item.label}</button>
          ))}
          <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => editor.chain().focus().deleteTable().run()} className="rounded px-2 py-1 text-red-600 hover:bg-red-50">Delete Table</button>
        </div>
      )}
