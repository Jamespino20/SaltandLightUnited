"use client";

import Link from "next/link";
import { FacebookLogo } from "@phosphor-icons/react";
import { useSiteConfig } from "@/lib/useSiteConfig";
import { useTranslations } from "next-intl";
import { brand } from "@/lib/brand";

export function Footer() {
  const config = useSiteConfig();
  const t = useTranslations("footer");

  return (
    <footer className="relative isolate overflow-hidden border-t border-slu-gray-800 bg-slu-black text-white">
      {/* returning light at the very end of the journey */}
      <div
        aria-hidden
        className="absolute left-1/2 top-0 -z-10 h-64 w-64 -translate-x-1/2 rounded-full bg-amber-200/20 blur-3xl"
      />
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          {/* Brand */}
          <div>
            {/* SLU pill badge */}
            <div className="flex justify-start">
              <img
                src="/images/BlueWhitePill.svg"
                alt={config.siteName}
                className="h-12 w-auto opacity-90 drop-shadow-[0_0_24px_rgba(255,221,150,0.45)]"
              />
            </div>
            <p className="mt-2 text-sm text-slu-gray-400">
              {config.description}
            </p>
            <div className="mt-4 flex flex-col gap-2">
              <Link
                href={config.facebookUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-sm text-slu-gray-400 transition-colors hover:text-slu-blue"
              >
                <FacebookLogo size={20} />
                {t("followFacebook")}
              </Link>
              <Link
                href={brand.discordUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-sm text-slu-gray-400 transition-colors hover:text-[#5865F2]"
              >
                <svg viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5">
                  <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.095 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.095 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
                </svg>
                {t("joinDiscord")}
              </Link>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="text-sm font-semibold uppercase tracking-wider text-slu-gray-300">
              {t("quickLinks")}
            </h4>
            <ul className="mt-3 space-y-2">
              {[
                { href: "/about", label: t("aboutUs") },
                { href: "/events", label: t("events") },
                { href: "/groups", label: t("community") },
                { href: "/resources", label: t("resources") },
              ].map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-slu-gray-400 transition-colors hover:text-white"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Connect + Legal */}
          <div>
            <h4 className="text-sm font-semibold uppercase tracking-wider text-slu-gray-300">
              {t("connect")}
            </h4>
            <ul className="mt-3 space-y-2">
              <li>
                <Link
                  href="/contact"
                  className="text-sm text-slu-gray-400 transition-colors hover:text-white"
                >
                  {t("contactUs")}
                </Link>
              </li>
            </ul>

            <h4 className="mt-6 text-sm font-semibold uppercase tracking-wider text-slu-gray-300">
              {t("legal")}
            </h4>
            <ul className="mt-3 space-y-2">
              <li>
                <Link
                  href="/privacy"
                  className="text-sm text-slu-gray-400 transition-colors hover:text-white"
                >
                  {t("privacyPolicy")}
                </Link>
              </li>
              <li>
                <Link
                  href="/terms"
                  className="text-sm text-slu-gray-400 transition-colors hover:text-white"
                >
                  {t("termsOfService")}
                </Link>
              </li>
            </ul>
          </div>
        </div>

        {/* Bottom */}
        <div className="mt-8 border-t border-slu-gray-800 pt-6 text-center text-xs text-slu-gray-500">
          <p>
            &copy; {new Date().getFullYear()} {config.siteName}. {t("allRights")}
          </p>
          <p className="mt-1">{config.independenceNote}</p>
        </div>
      </div>
    </footer>
  );
}
